...
(defparameter *fuzzy-rules*
  (make-hash-table)
  (setf (gethash 1 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "MAHAL" :kelayakan (lambda (p h) (+ 10 (* 0.2 p) (* 0.1 h)))))
  (setf (gethash 2 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "SEDANG" :kelayakan (lambda (p h) (+ 50 (* 0.5 p) (* -0.1 h)))))
  (setf (gethash 3 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "MURAH" :kelayakan (lambda (p h) (+ 80 (* 0.3 p) (* -0.2 h)))))
  (setf (gethash 4 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "MURAH" :kelayakan (lambda (p h) (+ 20 (* 0.1 p) (* 0.1 h)))))
  (setf (gethash 5 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "MURAH" :kelayakan (lambda (p h) (+ 40 (* 0.4 p) (* -0.1 h)))))
  (setf (gethash 6 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "SEDANG" :kelayakan (lambda (p h) (+ 70 (* 0.2 p) (* 0.1 h)))))
  (setf (gethash 7 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "SEDANG" :kelayakan (lambda (p h) (+ 30 (* 0.3 p) (* -0.2 h)))))
  (setf (gethash 8 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "MAHAL" :kelayakan (lambda (p h) (+ 60 (* 0.6 p) (* -0.3 h))))
  (setf (gethash 9 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "MAHAL" :kelayakan (lambda (p h) (+ 90 (* 0.4 p) (* 0.2 h)))))))

; Function to perform fuzzy inference
(defun inferensi-fuzzy (kualitas-pelayanan-rendah kualitas-pelayanan-sedang kualitas-pelayanan-tinggi
                      harga-murah harga-sedang harga-mahal pelayanan harga)
  (let ((rule-outputs '()))
    ...
    (let ((pelayanan-inputs `(("RENDAH" ,kualitas-pelayanan-rendah)
          ...
          (harga-inputs `(("MURAH" ,harga-murah)
                          ("SEDANG" ,harga-sedang)
                          ("MAHAL" ,harga-mahal))))
      (loop for rule-num in (loop for i from 1 to 9 collect i)
            do (let* ((rule-data (gethash rule-num *fuzzy-rules*))
                       (pelayanan-condition (cdr (assoc (getf rule-data :pelayanan) pelayanan-inputs)))
                        (harga-condition (cdr (assoc (getf rule-data :harga) harga-inputs)))
                        (weight (if (member rule-num '(1 3 5 9))  ; Rules menggunakan OR
                           (max pelayanan-condition harga-condition)
                           ; Rules menggunakan AND
                           (min pelayanan-condition harga-condition)))
              ; Hitung output crisp dengan memanggil fungsi konsekuen
              (output-value (funcall (getf rule :kelayakan) pelayanan harga))) ; Calculate output using TSK function
                (push (list (cl-tuples:make-tuple weight output-value)) rule-outputs)))) ; Store weight and crisp output
    rule-outputs))
...
(defun defuzzifikasi (rule-outputs)
  (handler-case
      (let ((numerator 0)
            (denominator 0))
        (dolist (output rule-outputs)
          (let ((weight (car output))
                (output-value (cadr output)))
            (setf numerator (+ numerator (* weight output-value)))

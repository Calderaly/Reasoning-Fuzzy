...
(defparameter *fuzzy-rules*
  (make-hash-table)
  (setf (gethash 1 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "MAHAL" :kelayakan (:a 0 :b 0 :c 30)))
  (setf (gethash 2 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "SEDANG" :kelayakan (:a 30 :b 50 :c 70)))
  (setf (gethash 3 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "MURAH" :kelayakan (:a 70 :b 100 :c 100)))
  (setf (gethash 4 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "MURAH" :kelayakan (:a 0 :b 10 :c 20)))
  (setf (gethash 5 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "MURAH" :kelayakan (:a 20 :b 40 :c 60)))
  (setf (gethash 6 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "SEDANG" :kelayakan (:a 60 :b 80 :c 90)))
  (setf (gethash 7 *fuzzy-rules*) '(:pelayanan "RENDAH" :harga "SEDANG" :kelayakan (:a 10 :b 20 :c 40)))
  (setf (gethash 8 *fuzzy-rules*) '(:pelayanan "SEDANG" :harga "MAHAL" :kelayakan (:a 40 :b 60 :c 80)))
  (setf (gethash 9 *fuzzy-rules*) '(:pelayanan "TINGGI" :harga "MAHAL" :kelayakan (:a 80 :b 90 :c 100))))
...
(defun inferensi-fuzzy (kualitas-pelayanan-rendah kualitas-pelayanan-sedang kualitas-pelayanan-tinggi harga-murah harga-sedang 
  ...
  (let ((rule-outputs '()))
    ...
    (let ((pelayanan-inputs `(("RENDAH" ,kualitas-pelayanan-rendah)
      ...
      (loop for rule-num from 1 to 9 do
        (let* ((rule-data (gethash rule-num *fuzzy-rules*))
               ...
               (output-fuzzy-set (getf rule-data :kelayakan))) ; ambil himpunan fuzzy dari aturan
          (push (list (cl-tuples:make-tuple weight output-fuzzy-set)) rule-outputs)))) ; Simpan bobot dan himpunan fuzzy
    rule-outputs))

; Fungsi untuk melakukan defuzzifikasi (menggunakan metode Centroid untuk Tahani)
(defun defuzzifikasi (rule-outputs)
  "
  Melakukan defuzzifikasi menggunakan metode Centroid untuk logika Tahani.
  ...
  Args:
      rule_outputs (list): List of tuple berisi bobot aturan dan himpunan fuzzy outputnya.
          ...
          adalah dictionary {'a': a, 'b': b, 'c': c} yang merepresentasikan
      ...
      (let ((numerator 0)
        ...
        (dolist (output rule-outputs)
          (let ((weight (car output))
                (output-fuzzy-set (cadr output))
                (a (getf (cadr output) :a))
                (b (getf (cadr output) :b))
                (c (getf (cadr output) :c)))

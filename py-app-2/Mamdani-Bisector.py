...
def defuzzifikasi(kelayakan_rendah, kelayakan_sedang, kelayakan_tinggi):
    ...
    domain = list(range(0, 101))  # Domain kelayakan dari 0 hingga 100

    # Hitung area di bawah kurva untuk setiap kategori kelayakan
    area_rendah = sum(kelayakan_rendah if i <= 30 else 0 for i in domain)
    area_sedang = sum(kelayakan_sedang if 30 < i <= 70 else 0 for i in domain)
    area_tinggi = sum(kelayakan_tinggi if i > 70 else 0 for i in domain)

    total_area = area_rendah + area_sedang + area_tinggi

    # Jika total area nol, kembalikan nilai tengah domain
    if total_area == 0:
        return 50
    ...
    current_area = 0
    for i in domain:
        current_area += (kelayakan_rendah if i <= 30 else 0) + (kelayakan_sedang if 30 < i <= 70 else 0) + (kelayakan_tinggi if i > 70 else 0)
        if current_area >= total_area / 2:
            return i
    return 100  # Jika tidak ditemukan, kembalikan nilai maksimum

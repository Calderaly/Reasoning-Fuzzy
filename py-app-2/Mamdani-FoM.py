...
# Fungsi untuk melakukan defuzzifikasi (menggunakan metode First of Maximum - FOM)
def defuzzifikasi(kelayakan_rendah, kelayakan_sedang, kelayakan_tinggi):
    """
    Melakukan defuzzifikasi menggunakan metode First of Maximum (FOM).
    ...
    max_keanggotaan = max(keanggotaan_rendah, keanggotaan_sedang, keanggotaan_tinggi)

    # Prioritaskan kategori dalam urutan: rendah, sedang, tinggi
    if max_keanggotaan == keanggotaan_rendah:
        for i, val in enumerate(output_rendah):
            if keanggotaan_rendah:  # Asumsi keanggotaan_rendah sbg pengganti max_keanggotaan
                return output_rendah[i]
    elif max_keanggotaan == keanggotaan_sedang:
        for i, val in enumerate(output_sedang):
            if keanggotaan_sedang:
                return output_sedang[i]
    elif max_keanggotaan == keanggotaan_tinggi:
        for i, val in enumerate(output_tinggi):
            if keanggotaan_tinggi:
                return output_tinggi[i]

    return 50 # Default jika tidak ada yang memenuhi

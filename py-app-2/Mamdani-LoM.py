...
def defuzzifikasi(kelayakan_rendah, kelayakan_sedang, kelayakan_tinggi):
    ...
    if max_keanggotaan == keanggotaan_tinggi:
        for i, val in reversed(list(enumerate(output_tinggi))): # Iterate in reverse
            if keanggotaan_tinggi:
                return output_tinggi[i]
    elif max_keanggotaan == keanggotaan_sedang:
        for i, val in reversed(list(enumerate(output_sedang))):
            if keanggotaan_sedang:
                return output_sedang[i]
    elif max_keanggotaan == keanggotaan_rendah:
        for i, val in reversed(list(enumerate(output_rendah))):
            if keanggotaan_rendah:
